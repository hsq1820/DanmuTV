...
// ---- 常量 ----
const PLAY_RECORDS_KEY = 'danmutv_play_records';
const FAVORITES_KEY = 'danmutv_favorites';
const SEARCH_HISTORY_KEY = 'danmutv_search_history';

// 缓存相关常量
const CACHE_PREFIX = 'danmutv_cache_';
const CACHE_VERSION = '1.0.0';
const CACHE_EXPIRE_TIME = 60 * 60 * 1000; // 一小时缓存过期

// ---- 环境变量 ----
// 桌面应用强制使用 localStorage
...
export async function savePlayRecord(
  ...
  try {
    ...
    console.error('[DanmuTV] 保存播放记录失败:', err);
    triggerGlobalError('保存播放记录失败');
    throw err;
  }
}
...
export async function addSearchHistory(keyword: string): Promise<void> {
  ...
  try {
    ...
    console.error('[DanmuTV] 保存搜索历史失败:', err);
...
export async function getSkipConfig(
  ...
  // localStorage 模式
  try {
    const raw = localStorage.getItem('danmutv_skip_configs');
    if (!raw) return null;
...
export async function saveSkipConfig(
  ...
  try {
    ...
    configs[key] = config;
    localStorage.setItem('danmutv_skip_configs', JSON.stringify(configs));
    window.dispatchEvent(
      new CustomEvent('skipConfigsUpdated', {
        detail: configs,
      })
